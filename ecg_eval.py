...
import pandas as pd
import ast
from ecg_tool import ECGDataset
...
if __name__ == "__main__":
    ...
    # 3. 印出核心指標
    print(f"整體 Micro precision: {checkpoint.get('precision_micro', 0):.4f}")
    print(f"整體 Micro recall: {checkpoint.get('recall_micro', 0):.4f}")
    print(f"整體 Micro F1: {checkpoint.get('f1_micro', 0):.4f}")
    print(f"整體 Macro precision: {checkpoint.get('precision_macro', 0):.4f}")
    print(f"整體 Macro recall: {checkpoint.get('recall_macro', 0):.4f}")
